...
| File | Description |
| --- | --- |
| `package.json` | Single npm package: scripts for dev, build, typecheck, deploy, owner auth keys, seed |
| `scripts/configure-auth-keys.mjs` | Generates Convex Auth JWT keys and a Gmail token-encryption key and stores them directly in the selected deployment |
...
| `index.html` | Private NewRevGen Vite entry HTML with minimal metadata and the pre-paint theme script; no inherited analytics tracker |
...
| `schema.ts` | All tables: Convex Auth sessions/accounts, workspace, CRM users, Gmail connection and OAuth state, companies, contacts, deals, activities, custom fields, agent tasks, agents, runs, facts, chats, logs, and Slack identities |
| `auth.config.ts` | Configures Convex to validate Convex Auth tokens issued by this deployment |
| `auth.ts` | Password provider, 12-character password rule, and the owner-email allowlist callback |
| `setup.ts` | Environment-backed one-time owner initializer; creates/locks the workspace to `OWNER_EMAIL` |
...
| `ai.ts` | AI provider registry: OpenAI (default), Anthropic, OpenRouter, DeepSeek, and Grok models, key checks, and the missing-key reply |
...
| `email.ts` | Gmail, Resend, and AgentMail delivery selection, compose defaults, timeline logging, attachment upload URLs, and notification delivery |
...
| `model/access.ts` | Read and write access checks; requires both a session and an email in the single-workspace allowlist |
| `model/workspace.ts` | Shared blank NewRevGen workspace creation used by setup and first-boot helpers |
...
| File | Description |
| --- | --- |
| `main.tsx` | Convex Auth provider, Convex client, and router setup; derives the backend URL when served from convex.site |
| `App.tsx` | Authentication gate and routes for the CRM shell, compare, and docs |
| `auth/SignInPage.tsx` | Single-owner sign-in and initial owner-account creation form |
