...
function TeamSection() {
  ...
  return (
    <Panel className="p-4">
      ...
      <div className="mb-3 rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3 text-xs leading-relaxed text-emerald-200/90">
        <span className="font-medium">Private owner access.</span> Convex checks
        both the signed-in session and the workspace email allowlist before
        returning or changing CRM data.
      </div>
      ...
      <p className="mt-4 text-xs leading-relaxed text-neutral-600">
        This workspace is intentionally single-owner. Change OWNER_EMAIL and
        rerun the owner initializer only if you are transferring ownership.
      </p>
    </Panel>
  );
}
...
function EmailSection() {
  ...
  const disconnectGmail = useAction(api.gmail.disconnect);
  ...
  const connectGmail = async () => {
    ...
    setGmailError(null);
    try {
      window.location.assign(await beginGmailConnection());
    } catch (error) {
      setGmailError(
  ...
  const removeGmail = async () => {
    if (!window.confirm("Disconnect Gmail and revoke the CRM's send access?")) {
      return;
    }
    setGmailBusy(true);
    setGmailError(null);
    ...
    <div className="flex flex-col gap-4">
      <Panel className="p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="max-w-2xl text-xs leading-relaxed text-neutral-500">
              Connect your own mailbox with Google OAuth. The CRM requests only
              permission to send email; it does not read, move, or delete your
              inbox. The refresh token is encrypted before it is stored in
              Convex.
            </p>
          </div>
          {gmail?.connected ? (
        ...
        {gmail?.connected ? (
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <span className="text-sm text-neutral-200">{gmail.email}</span>
            <Button
              variant="danger"
              disabled={gmailBusy}
              onClick={() => void removeGmail()}
            >
        ...
        {gmailResult === "connected" ? (
          <p className="mt-3 text-xs text-emerald-400">
            Gmail connected. It is now the active email provider.
          </p>
        ) : null}
        {gmailResult === "denied" ? (
          <p className="mt-3 text-xs text-amber-300">
            Google access was cancelled. Nothing was connected.
          </p>
        ) : null}
        {gmailResult === "error" || gmailError ? (
          <p className="mt-3 text-xs text-red-400">
            {gmailError ??
              "Google could not complete the connection. Check the OAuth redirect URI and Convex environment variables."}
          </p>
        ) : null}
        <p className="mt-4 text-[11px] leading-relaxed text-neutral-600">
          Google OAuth callback for this URL: {window.location.origin}
      ...
      <Panel className="p-4">
        ...
        <p className="mb-3 text-xs leading-relaxed text-neutral-500">
          ...
          provider. Gmail sends from your connected account, Resend is plain
          outbound email, and AgentMail also gives agents a persistent inbox.
          The
        ...
        <div className="flex gap-2">
          {(["gmail", "resend", "agentmail"] as const).map((option) => {
      ...
      <Panel className="p-4">
        ...
        <p className="mb-4 text-xs leading-relaxed text-neutral-500">
          ...
          window. These fields set the display name and signature that append
          to every message. With Resend the from address must belong to a
          verified domain. Gmail and AgentMail send from their connected inbox
          addresses, so they ignore the custom from address.
        </p>
