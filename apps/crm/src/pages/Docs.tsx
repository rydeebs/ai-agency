...
const SECTIONS = [
  ...
  { id: "email", label: "Email: Gmail, Resend, AgentMail" },
...
const ENV_ROWS: Array<{
...
}> = [
  {
    name: "OWNER_EMAIL",
    required: "Yes",
    enables: "The single email allowed to create a CRM session",
    where: "Your own email address",
  },
  {
    name: "JWT_PRIVATE_KEY + JWKS",
    required: "Yes",
    enables: "Convex Auth signing and verification",
    where: "Generated by npm run auth:keys",
  },
  {
    name: "GMAIL_CLIENT_ID + GMAIL_CLIENT_SECRET",
    required: "Only for Gmail",
    enables: "Google OAuth and sending from your connected Gmail account",
    where: "Google Cloud Credentials",
  },
  {
    name: "GMAIL_TOKEN_ENCRYPTION_KEY",
    required: "Only for Gmail",
    enables: "Encrypts the Gmail refresh token before database storage",
    where: "Generated by npm run auth:keys",
  },
  {
...
npx convex env set EXA_API_KEY unset`}</Code>
          ...
          <Section id="email" title="Email: Gmail, Resend, or AgentMail">
            <p>
              ...
              compose window. Three providers are wired in and you pick one in
              Settings. They stay off until configured, and the app logs
              what it would have sent instead of failing.
            </p>
            <ul className="flex list-disc flex-col gap-2 pl-5">
              <li>
                <span className="text-white">Gmail</span> sends through your
                own connected Google account. Enable the Gmail API, create a
                Web OAuth client, add your deployment's
                /oauth/gmail/callback URL as an authorized redirect URI, set
                <K>GMAIL_CLIENT_ID</K> and <K>GMAIL_CLIENT_SECRET</K>, then
                click Connect Gmail in Settings, Email. The CRM requests only
                the <K>gmail.send</K> scope; it does not read, move, or delete
                inbox messages.
              </li>
              <li>
...
npx convex env set RESEND_API_KEY re_... --prod  # production`}</Code>
            ...
            <p>
              ...
              . Gmail and AgentMail always send from their connected inbox
              addresses, so the custom from address applies to Resend only.
            </p>
          </Section>
...
npx convex env set XAI_API_KEY xai-...`}</Code>
          ...
          <Section id="auth" title="Turning on sign-in">
            <p>
              This fork uses Convex Auth with email and password. The frontend
              hides the app until a session exists, and backend reads and
              writes independently check the workspace owner allowlist.
            </p>
            <p>
              Convex Auth runs in the same deployment; there is no separate
              authentication service. See the{" "}
              <Ext href="https://docs.convex.dev/auth/convex-auth">
                Convex Auth
              </Ext>
              {" "}documentation for its session model. Initialize a deployment
              with the commands below, substituting your email.
            </p>
            <Code>{`npm run auth:keys
npx convex env set OWNER_EMAIL you@example.com
npx convex run setup:initializeOwner`}</Code>
            <p>
              Open the site and choose Create owner login once, using that
              same address and a password of at least 12 characters. Empty
              allowlists reject everyone. Production uses separate signing
              keys and its own OWNER_EMAIL value.
            </p>
          </Section>
...
npx convex env set EXA_API_KEY unset --prod`}</Code>
            ...
            <p>
              ...
              paths like <K>/compare</K> and <K>/docs</K> work on refresh.
            </p>
            <Code>{`npm run auth:keys:prod -- --site-url=https://YOUR-PRODUCTION.convex.site
npx convex env set OWNER_EMAIL you@example.com --prod
npx convex run setup:initializeOwner --prod`}</Code>
            <p>
