...
This NewRevGen fork has no demo-reset cron. `setup:initializeOwner` creates or
updates the single workspace, sets `demoMode` to false, and copies the
deployment's `OWNER_EMAIL` into `workspace.allowedSignIn`. Do not reintroduce a
reset over real CRM data.
...
## Authentication and mailbox boundaries
...
**Auth.** Convex Auth is installed with the Password provider. Sessions are
gated in `convex/auth.ts` by `workspace.allowedSignIn`, and backend reads and
writes repeat that allowlist check in `convex/model/access.ts`. Empty means
nobody signs in. `OWNER_EMAIL` is applied through `setup:initializeOwner`.
Convex Auth does not persist third-party API refresh tokens.

**Mailbox sync.** There is no `mailboxConnections` table and no inbound mail
sync. The
...
email through Gmail, Resend, or AgentMail, which does exist and does send.
