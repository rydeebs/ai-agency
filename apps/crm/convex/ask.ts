...
async function runSlashCommand(
  ...
  const emailReady =
    emailProvider === "gmail"
      ? (await ctx.db.query("gmailConnections").first()) !== null
  ...
  if (remindMe) {
    if (!reminderScheduled) {
      ...
      lines.push(
        `A reminder email is scheduled via ${emailProviderLabel}.`,
      );
    } else {
      lines.push(
        "A reminder is scheduled, but the selected email provider is not connected. Connect Gmail or configure the Resend or AgentMail keys to enable it.",
      );
    }
  }
